...
class Server
{
    ...
    float time_arrival [Q_LIMIT_1+1], time_last_event, time_next_event[3], total_delay,delay_S1,delay_S2;
public:
    void main_program();
    void initialize();
    void timing();
...
void Server::report()
{
    cout<<"\n  Simulation clock time= "<<sim_time;
    ...
    cout<<"\n  Server 1 (Petrol) utilization= "<<area_server_status_1/sim_time;
    cout<<"\n  Server 2 (Petrol) utilization= "<<area_server_status_2/sim_time;
    float Fuel;
    Fuel=fuel_consumption(num_delays_required);
    cout<<"\n  Total fuel consumption (Petrol)= "<<Fuel<<endl;
}
